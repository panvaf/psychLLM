...
def generate_heatmap(user_id, user_data, output_dir):
    ...
    kl_data = np.log(kl_data)

    # Debug: Check for small values that might be rounded to zero
    min_val = kl_data.min().min()
    print(f"User: {user_id} - Minimum log(KL divergence): {min_val}")
    ...
    sns.heatmap(
        kl_data,
        annot=True,
        fmt=".3f",  # Increased to three decimal places
        cmap='coolwarm',
        cbar_kws={'label': 'Logarithm of KL Divergence'},
        linewidths=.5,
        linecolor='gray'
    )
    plt.title(f'Logarithm of KL Divergence Over Time for {user_id}')
    ...
    heatmap_path = os.path.join(user_output_dir, f'{user_id}_kl_divergence_log_heatmap.png')
...
def main():
    ...
    output_dir = os.path.join(project_root, "data", "heatmaps", "log")
